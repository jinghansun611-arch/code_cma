...
import time
import os
# ------------------ Type-safe scalar conversion (fix numpy.uint8 / ndarray for Abaqus BC) ------------------
def _to_float(x, default=0.0):
    # Convert MATLAB-loaded numpy scalars/arrays (e.g., numpy.uint8, (1,1) ndarray) to Python float
    # Abaqus setValuesInStep does NOT accept numpy scalar types.
    try:
...
#abaqus open
#How to use: Matlab
#system('abaqus cae noGUI=Sup_cae_create.py')

#cae_save:in the end
#mdb.saveAs("D:\Abaqus DATA\Design-Support\A1\TESETA")
...
General = str(mat_cae['NUM_T'][0][0])
num_Phe = int(mat_cae['N'][0][0])
jobnum  = int(mat_cae['multitask'][0][0])
cpus    = int(mat_cae['cpus'][0][0])

#data read
for i in range(1, num_Phe + 1):
    name_mat = 'mat-' + General +'-' + str(i) + '.mat'
    ...
    for kl in range(0,NUM_LINE):
        ...
        for j in range(0,np.size(PP,0)):
            ...
            Part_surname = name='P1-Bond-' + str(kl+1) + '-' + str(j+1) 
            M.SurfaceToSurfaceContactStd(
                adjustMethod=NONE, 
                clearanceRegion=None, 
                createStepName='Initial', 
    ...
    M.boundaryConditions['BC-Surface'].setValuesInStep(stepName='Step-1', 
        u3= P2_AZ ) #P2_AZ
    M.boundaryConditions['BC-Surface'].setValuesInStep(stepName='Step-2', 
        u3= 0 ) #sjh新增 让BASE回到原位以免对step3开始释放造成影响
    ...
    for kl in range(0,NUM_LINE):
        ...
        for j in range(0,np.size(PP,0)):
            ...
            M.boundaryConditions[BC_name].setValuesInStep(
                stepName='Step-3', 
                u1  = _to_float(BOND_U[kl][0][j][0]), 
                u2  = _to_float(BOND_U[kl][0][j][1]), 
                u3  = _to_float(BOND_U[kl][0][j][2]),
                ur1 = _to_float(BOND_U[kl][0][j][3]), 
                ur2 = _to_float(BOND_U[kl][0][j][4]), 
                ur3 = _to_float(BOND_U[kl][0][j][5]) )
            M.boundaryConditions[BC_name].setValuesInStep(
                stepName='Step-4',  
                u1  = _to_float(BOND_U[kl][1][j][0]), 
                u2  = _to_float(BOND_U[kl][1][j][1]), 
                u3  = _to_float(BOND_U[kl][1][j][2]),
                ur1 = _to_float(BOND_U[kl][1][j][3]), 
                ur2 = _to_float(BOND_U[kl][1][j][4]), 
                ur3 = _to_float(BOND_U[kl][1][j][5]) )
...
#----------------------------------Job
    mdb.Job(atTime=None, contactPrint=OFF, description='', echoPrint=OFF, 
            ...
            memory=90, memoryUnits=PERCENTAGE, model=NAME_MODEL, modelPrint=OFF, 
            multiprocessingMode=THREADS, name= NAME_JOB , nodalOutputPrecision=
            SINGLE, numCpus=cpus, numDomains=cpus, numGPUs=0, queue=None, resultsFormat=ODB
            , scratch='', type=ANALYSIS, userSubroutine='', waitHours=0, waitMinutes=0)
    
if 1==1:
    job_order=1
    while job_order<=num_Phe:
        if job_order+jobnum<=num_Phe:
            for i in range(0,jobnum):
                NAME_JOB=str('Job-'+General+'-'+str(job_order+i))
