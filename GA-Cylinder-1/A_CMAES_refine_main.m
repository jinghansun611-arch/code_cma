...
% ========== 0) 【精修建议参数】你只需要在这里改 ==========
GA.T_MAX  = 50;          % 精修迭代数（建议 20~40）
GA.N      = 20;           % 精修每代个体数（建议 6~10；你机器也可 10~12）
GA.TYPE  = 5;
GA.D     = [11,11,1,1,1];
GA.EXPECT = 0.01;        % 精修期望误差
GA.BIT    = 10000;

GA.CMA.sigma0 = 0.08;    % ★精修核心：步长要小（0.05~0.10）
...
stage1_dir = fullfile(root_dir, ['A-CMAES-7S-1-80-25-refine1']);  % e.g. ...\A-CMAES-2S
bestfile_stage1 = fullfile(stage1_dir, 'best_stage1.mat');

if ~exist(bestfile_stage1, 'file')
    error(['找不到 Stage-1 best_stage1.mat：\n  %s\n' ...
           '请确认：1) 确实先跑过粗搜；2) GA.ORDER 对应的文件夹存在；3) 路径无误。'], bestfile_stage1);
end
...
% 维度检查：必须与当前 map.nvars 一致（否则说明改了 symmetry/JUDGE/range）
