...
% ========= 统一定义各 TYPE 的开关语义 ========= 3.10添加 为了更清晰
% TYPE1: 当前代码中默认永远开启（JUDGE(1,:) 暂未真正接入）
type1_on = true;

% TYPE2: 按条带分别控制
type2_on = @(k) (size(JUDGE,1) >= 2 && size(JUDGE,2) >= k && JUDGE(2,k) == 1);

% TYPE3: 全局单标量，仅单条带时有效，只读取 JUDGE(3,1)
type3_on = (num_line == 1 && size(JUDGE,1) >= 3 && size(JUDGE,2) >= 1 && JUDGE(3,1) == 1);

% TYPE4: 全局单标量，建议只读取 JUDGE(4,1)
type4_on = (size(JUDGE,1) >= 4 && size(JUDGE,2) >= 1 && JUDGE(4,1) == 1);

% TYPE5: 全局单标量，建议只读取 JUDGE(5,1)
type5_on = (size(JUDGE,1) >= 5 && size(JUDGE,2) >= 1 && JUDGE(5,1) == 1);
...
x_tw0 = 0.0;
L_tw  = 0.0;
if TYPE >= 5 && size(range,1) >= 5 && type5_on
    use_twist = true;
...
%  [5] : 位移边界条件信息（左Bonding点始终为固定）
    ...
    fix_left_ux = false; 
    fix_left_urx = false;  % 手动控制 平时是false 只有目标构型左侧和草图左侧位于同一轴线上不再需要扭转时开启
